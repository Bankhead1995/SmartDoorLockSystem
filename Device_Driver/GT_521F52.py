...
import serial
import struct
from Device_Driver import packets
import time
import logging
...
#TODO: FIX -> After updating the baudrate the serial cant communicate with the sensor
    ...
    def changeFingerPrint(self,user_id):
            logging.debug("changeFingerPrint started.")
            self.Enroll_Start(user_id)
            ...
            for i in range(1,4):
                ...
                self.enroll(i)
                time.sleep(1)
            logging.debug("changeFingerPrint ended.")
